...
def build_kb():
    ...
    kb = {
        'P_implies_Q': 'not P or Q',           # P → Q
        'Q_implies_R': 'not Q or R',           # Q → R
        'Main_Expression': '(not P or Q) and (not Q or R) and P',   # (not P or Q) and (not Q or R) and P
        'Q_or_R': 'Q or R'                     # Q ∨ R
    }
    return kb
...
def check_entailment(kb_true_rows, expr):
    """Check if the KB entails a given expression."""
    entails = True
...
def main():
    ...
    # Display the knowledge base clauses
    ...
    # Predefined additional sentences to check entailment
    ...
    # Check entailment for predefined sentences
